...
class Config:
    def __init__(self):
        ...
        # Set API keys
        self.financial_datasets_api_key = os.environ.get("FINANCIAL_DATASETS_API_KEY", "")
        
        # Directory where server.py is located
        self.financial_datasets_server_dir = self.repo_root / "mcp_servers" / "financial-datasets"
    ...
    def _validate_config(self):
        ...
        try:
            ...
            if result.returncode != 0:
                raise EnvironmentError("Docker is required but doesn't appear to be installed or running")
        except FileNotFoundError:
            raise EnvironmentError("Docker is required but doesn't appear to be installed")
        
        # Check if uv is installed for fredapi and financial-datasets
        try:
            result = subprocess.run(["uv", "--version"], capture_output=True, text=True)
            if result.returncode != 0:
                print("Warning: 'uv' command not found. Please install uv or ensure it's in your PATH.")
        except FileNotFoundError:
            print("Warning: 'uv' command not found. Please install uv or ensure it's in your PATH.")
            
        # Check if fredapi directory exists
        if not self.fredapi_server_dir.exists():
            raise FileNotFoundError(f"FRED API server directory not found: {self.fredapi_server_dir}")
            
        # Check if financial-datasets directory exists
        if not self.financial_datasets_server_dir.exists():
            raise FileNotFoundError(f"Financial datasets server directory not found: {self.financial_datasets_server_dir}")
        
        # Check for API keys
        if not self.fred_api_key:
            print("Warning: FRED_API_KEY is not set. FRED API functionality may be limited.")
            
        if not self.perplexity_api_key:
            print("Warning: PERPLEXITY_API_KEY is not set. Perplexity Search functionality may be limited.")
            
        if not self.financial_datasets_api_key:
            print("Warning: FINANCIAL_DATASETS_API_KEY is not set. Financial data functionality may be limited.")
