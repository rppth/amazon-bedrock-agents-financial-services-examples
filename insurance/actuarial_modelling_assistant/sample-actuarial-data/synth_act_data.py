...
def generate_policy_data(num_policies):
    ...
    start_dates = [datetime(2019, 1, 1) + timedelta(days=random.randint(0, 1460)) for _ in range(num_policies)]
    ...
    end_dates = [start + timedelta(days=int(duration * 30)) for start, duration in zip(start_dates, durations)]
    
    # Product types
    product_types = np.random.choice(['Auto', 'Home', 'Life', 'Health'], size=num_policies, p=[0.4, 0.3, 0.2, 0.1])
    
    # Geographic regions
    regions = np.random.choice(['North', 'South', 'East', 'West', 'Central'], size=num_policies)
...
def generate_claims_data(policies_df):
    ...
    for _, policy in policies_df.iterrows():
        ...
        risk_adjustment = policy['risk_score'] / 50  # normalize to have 1.0 at risk_score = 50
        ...
        if policy['product_type'] == 'Life':
            ...
            policy_duration = (policy['end_date'] - policy['start_date']).days / 365  # in years
        ...
        for _ in range(num_claims):
            ...
            if policy['product_type'] == 'Auto' and claim_date.month in [12, 1, 2]:  # Winter months
                amount *= 1.2  # Higher claims in winter
            elif policy['product_type'] == 'Home' and claim_date.month in [6, 7, 8, 9]:  # Summer/hurricane season
            ...
            # Claim type
            if policy['product_type'] == 'Auto':
                claim_types = ['Collision', 'Theft', 'Vandalism', 'Injury', 'Property Damage']
                claim_type = np.random.choice(claim_types, p=[0.5, 0.1, 0.1, 0.2, 0.1])
            elif policy['product_type'] == 'Home':
                claim_types = ['Fire', 'Theft', 'Water Damage', 'Liability', 'Natural Disaster']
                claim_type = np.random.choice(claim_types, p=[0.1, 0.2, 0.4, 0.1, 0.2])
            elif policy['product_type'] == 'Life':
                claim_type = 'Death Benefit'
            else:  # Health
                claim_types = ['Illness', 'Surgery', 'Accident', 'Preventive Care', 'Chronic Condition']
...
def generate_risk_factors(policies_df):
    ...
    for _, policy in policies_df.iterrows():
        ...
        if policy['product_type'] == 'Auto':
            ...
            # Prior accidents
            prior_accidents = int(np.random.poisson(max(0.2, (100 - policy['risk_score']) / 100)))
    ...
    for _, policy in policies_df.iterrows():
        ...
        for i, payment_date in enumerate(payment_dates):
            ...
            if i == 0:
                ...
                if policy['risk_score'] > 80:  # Higher risk clients more likely to miss payments
...
def generate_reserve_adjustments(claims_df):
    ...
    for _, claim in open_claims.iterrows():
        ...
        if days_range > 1 and num_adjustments < days_range:
            adjustment_days = sorted(np.random.choice(range(1, days_range), 
                                                    size=min(num_adjustments, days_range-1), 
                                                    replace=False))
            adjustment_dates = [claim_date + timedelta(days=int(day)) for day in adjustment_days]
            ...
            for i, adj_date in enumerate(adjustment_dates):
                ...
                if i < len(adjustment_dates) - 1:
                    ...
                    if claim['status'] == 'Litigation':
                        ...
                        expected_payout = claim['claim_amount'] * np.random.normal(1.2, 0.2)
                    else:
                        expected_payout = claim['claim_amount'] * np.random.normal(0.95, 0.1)
