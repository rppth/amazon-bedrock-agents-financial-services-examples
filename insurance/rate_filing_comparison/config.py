...
class Config:
    ...
    def _setup_bedrock_kb_search(self):
        ...
        self.bedrock_kb_search_dir = self.repo_root / "mcp_servers" / "bedrock-kb-search"
        ...
        self.bedrock_kb_search_params = StdioServerParameters(
            ...
            args=[
                "--directory",
                str(self.bedrock_kb_search_dir),
                "run",
                "server.py"
            ],
            env={
    ...
    def _setup_filesystem(self):
        ...
        for mount in mount_points:
            docker_mount_args.extend(["--mount", f"type=bind,src={mount['src']},dst={mount['dst']}"])
    ...
    def _validate_config(self):
        ...
        if not self.bedrock_kb_search_dir.exists():
            raise FileNotFoundError(f"Bedrock KB Search server directory not found: {self.bedrock_kb_search_dir}")
            
        # Check if the filesystem directory exists
        if not self.filesystem_dir.exists():
            raise FileNotFoundError(f"Filesystem MCP directory not found: {self.filesystem_dir}")
        ...
        try:
            ...
            if result.returncode != 0:
                print("Warning: Docker is required for the Filesystem MCP but doesn't appear to be installed or running")
        except FileNotFoundError:
            print("Warning: Docker is required for the Filesystem MCP but doesn't appear to be installed")
