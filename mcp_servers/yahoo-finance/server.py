...
@mcp.tool()
def get_options_data(ticker: str, date: Optional[str] = None) -> Dict[str, List[Dict[str, Any]]]:
    """
    ...
    if date is None:
        ...
        return {"error": f"Invalid expiration date. Available dates: {', '.join(expirations)}"}
...
def get_major_holders(ticker: str) -> Dict[str, Any]:
    ...
    result = {
        "major_holders": _dataframe_to_dict(major_holders) if major_holders is not None else None,
        "institutional_holders": _dataframe_to_dict(institutional_holders) if institutional_holders is not None else None,
        "mutualfund_holders": _dataframe_to_dict(mutualfund_holders) if mutualfund_holders is not None else None
    }
...
@mcp.tool()
def compare_stocks(tickers: List[str], metric: str = "price", period: str = "1mo") -> Dict[str, Any]:
    """
    ...
    if metric == "price":
        ...
        for ticker in tickers:
            ...
            result["data"][ticker] = {
                ...
                "change": round((ticker_data.dropna().iloc[-1] / ticker_data.dropna().iloc[0] - 1) * 100, 2)
            }
        ...
        for ticker in tickers:
            ...
            result["data"][ticker] = {
                ...
                "total_return": round((ticker_data.iloc[-1] / ticker_data.iloc[0] - 1) * 100, 2)
            }
...
def get_ticker_info_resource(ticker: str) -> str:
    ...
    report += f"**52 Week Range:** ${summary['52 Week Low']} - ${summary['52 Week High']}\n\n"
