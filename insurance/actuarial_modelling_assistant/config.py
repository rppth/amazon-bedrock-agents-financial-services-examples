...
class Config:
    ...
    def _load_env_vars(self):
        ...
        python_repl_env_file = self.project_root / "mcp_servers" / "python-repl" / ".env"
    ...
    def _validate_config(self):
        ...
        if not self.python_repl_dir.exists():
            raise FileNotFoundError(f"Python REPL MCP directory not found: {self.python_repl_dir}")
            
        # Check if sample data directory exists and list contents
        sample_data_dir = self.current_dir / "sample-actuarial-data"
        if not sample_data_dir.exists():
            raise FileNotFoundError(f"Sample actuarial data directory not found: {sample_data_dir}")
        else:
